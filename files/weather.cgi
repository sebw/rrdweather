...
# Fetching version from config file
my $config = Config::File::read_config_file('/etc/rrdweather.conf');
$config->{VERSION} =~ s/^"|"$//g;
my @ver = split(/ /, $config->{VERSION});
my $VERSION = $ver[0];
...
# Make an array with list of zips monitored
$config->{ZIPS} =~ s/^"|"$//g;
my @zips = split(/ /, $config->{ZIPS});

# If no ZIP as argument, display graphs of first ZIP entry in config file
if ( ! $zip ) {
	$zip = $zips[0];
}

# we need to split the unit from the zip
foreach $z (@zips){
        my @zip_array = split(/-/, $z);
        my $unit = $zip_array[0];
        my $zip = $zip_array[1];
	...
	sleep 3;
		if ( ! -r $xmlfile) {
			...
			print "The XML source may temporarily be unavailable. The page will reload after 5 seconds.\n\n";
			print "If the problem persists, there is probably a problem with this configuration.";
			exit 1;
		}
	}
	...
	HEADER
		...
		print "<form action=\"./weather.cgi\" method=\"get\">\n";
		print "<select name=\"zip\">";
		foreach(@zips){
			my $xml2 = new XML::Simple;
			my $xmlfile2 = "$tmp_dir/$zips[$n].xml";
			if ($xmlfile2) {
				my $xmldata2 = $xml2->XMLin("$xmlfile2");
				my $city2 = $xmldata2->{loc}->{dnam};
				print "<option value=\"$zips[$n]\">$city2</option>\n";
			}
			$n++;
		}
		print "</select>";
		print " ";
		print "<input type=\"submit\" value=\"Display graphs\" />";
		print "</form>";
		...
		for my $n (0..$#graphs) {
			...
			$h2_id =~ s/ /_/;
			if (not defined($next_h2_id)) {
				$next_h2_id = $graphs[0]{title};
			}
			$next_h2_id =~ s/ /_/;
	...
	<tr>\n
	<td>All time data are those of server location.<br /><br /></td>\n
	</tr>\n
	<tr>\n
	<td align="center"><a href="http://www.wains.be/projects/rrdweather/" onclick="window.open(this.href); return false;">RRDWeather</a> $VERSION by <a href="http://www.wains.be/" onclick="window.open(this.href); return false;">Sebastien Wains</a></td>\n
	</tr>\n
	</table>\n
	</body>\n
	</html>\n
